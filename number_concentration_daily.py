...
files = list_files(dir_tree, "one", ".dat")
df = pd.read_table(get_path(dir_tree, files[0]), skiprows=10, index_col=0)
nano = df.iloc[:, 1:9]
nano.insert(
    loc=0,
    column="total nano",
...
plt.plot(
    df.index, df["total counts"], "k.:", linewidth=1, label="Total particles"
)
plt.plot(
...
plt.ylabel("Number concentration [$\mathregular{1/cm^3}$]", **label_font)
plt.yticks(**tick_font)

# Calculate y-axis limits
ylower = 100 if df["total counts"].max() > 4e4 else 0
coeff = 1000 if df["total counts"].max() < 4e4 else 1e6
...
if save_figure.lower() != "n":
    fig_name = f'Total Number and Nano ({files[0][:files[0].rfind("-")]})'
    fig_path = (
        get_path(dir_tree, tell_parent(get_path(dir_tree, files[0])))
        + f"\\{fig_name}.png"
    )
