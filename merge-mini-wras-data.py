...
# DataFrame with measurement data for total and nano (heading skipped).
# Indexes are in the data (col=0) - timestamps - conversion needed.
for file in files:
    ...
    nano = df.iloc[:, 1:9]
    nano.insert(loc=0, column='total nano',
                value=[nano.iloc[i].sum() for i in range(nano.shape[0])])

    # Ensure that the file is overwritten
    if files.index(file) == 0:
        df.to_csv(path + r'\merged-data\total.csv', mode='w')
        nano.to_csv(path + r'\merged-data\nano.csv', mode='w')
    else:
        df.to_csv(path + r'\merged-data\total.csv', mode='a', header=False)
        nano.to_csv(path + r'\merged-data\nano.csv', mode='a', header=False)